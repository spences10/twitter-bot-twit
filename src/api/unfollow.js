...
const getFollowers = (callback, cursor, followers) => {
  if (!followers)
    followers = []
  client.get('followers/list', {screen_name: config.twitterConfig.username.screen_name, count: 200, include_user_entities: false, cursor}, (err, response) => {
    ...
    if (response.next_cursor && response.next_cursor > 0)
      ...
      return callback(undefined, _.uniq(followers, (user, key) => user.id))
  })
};
